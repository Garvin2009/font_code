...
class FCLIPLoss(nn.Module):
    ...
    @torch.no_grad()
    def eval_metrics(self, x):
        """
        純評估用，不參與梯度、不含 scheduler 權重、不做 augmentation。
        傳入乾淨的最終字形 tensor x (NCHW)，回傳 dict：
          - dir_cos : cos(Δimage, Δtext)，方向對齊度，範圍 -1~1，越高=越貼合「ref→target」的語意變化方向
          - abs_cos : cos(image, text)，優化後字形與目標 prompt 的絕對相似度，越高=越接近目標屬性
        只有在 use_fclip_direction_loss=True 時 dir_cos 才有意義（需要 ref）。
        """
        embedded_x = self.clip_model.encode_image(self.preprocess(x))
        metrics = {}
        # 絕對相似度：與目標 prompt 文字
        if getattr(self, "embedded_text", None) is not None:
            abs_cos = torch.cosine_similarity(
                embedded_x, self.embedded_text[:embedded_x.shape[0]], dim=-1
            ).mean().item()
            metrics["abs_cos"] = abs_cos
        # 方向對齊度：(優化後 - 原始) vs (目標prompt - 參考prompt)
        if self.use_fclip_direction_loss:
            delta_embedded_x = embedded_x - self.embedded_image_ref
            dir_cos = torch.cosine_similarity(
                delta_embedded_x, self.delta_embedded_text, dim=-1
            ).mean().item()
            metrics["dir_cos"] = dir_cos
        return metrics
...
class ConformalLoss:
    ...
    def get_angles(self, points: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(len(self.faces)):
            triangles = points[self.faces[i].to(points.device)]
            triangles_roll_a = points[self.faces_roll_a[i].to(points.device)]
