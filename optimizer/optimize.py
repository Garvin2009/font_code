...
def train(cfg: Config, signature: str):
    ...
    # --- [新增] 紀錄初始的控制點位置，作為變形的參考基準 ---
    init_points = torch.cat([p.clone().detach() for p in parameters["point"]], dim=0)
    # ----------------------------------------------------
    # training loop
    ...
    for step in t_range:
        ...
        if cfg.fclip_loss:
            ...
            #    loss = loss + loss_fclip
                ...
                # 1. 安全讀取開關：去 cfg 找 use_patch_based_loss，找不到就預設為 False (全圖)
                use_patch = getattr(cfg, 'use_patch_based_loss', False)

                img_for_clip = x_aug if cfg.use_aug else x

                if use_patch:
                    # =========================================================
                    # 【局部裁切模式】 (適合 Frantic 狂草)
                    # =========================================================
                    import torch.nn.functional as F
                    import torchvision.transforms.functional as TF
                    import random
                    
                    # 1. 計算整張圖的 CLIP Loss (維持原本看整體的邏輯)
                    loss_fclip_global = fclip_loss(img_for_clip, step=None)
                    
                    # 2. 計算局部裁切的 CLIP Loss (強迫看局部細節)
                    loss_fclip_patches = 0
                    _, _, h_img, w_img = img_for_clip.shape
                    patch_size = int(h_img * 0.6) # 每次隨機裁切 60% 大小的區塊
                    num_patches = 4           # 每次迭代切 4 塊來檢查
                    ...
                    for _ in range(num_patches):
                        ...
                        # 切割圖片
                        patch = TF.crop(img_for_clip, top, left, patch_size, patch_size)
                        # 將切下來的局部圖，放大回原本大小讓 CLIP 讀取
                        patch_resized = F.interpolate(patch, size=(h_img, w_img), mode='bilinear', align_corners=False)
                        loss_fclip_patches += fclip_loss(patch_resized, step=None)
                    
                    # 3. 將全域與局部的 Loss 結合 (局部權重先給 0.8，避免力道過猛)
                    loss_fclip = loss_fclip_global + 0.8 * (loss_fclip_patches / num_patches)
                    loss = loss + loss_fclip
                    # =========================================================

                else:
                    # =========================================================
                    # 【正常全圖模式】 (適合 Italic, Formal 幾何排版)
                    # =========================================================
                    loss_fclip = fclip_loss(img_for_clip, step=None)
                    loss = loss + loss_fclip
                    # =========================================================
        ...
        # --- [新增] 全域位移正則化 (Global Displacement Loss) ---
        # 這裡使用 getattr 是為了避免你以前的舊腳本沒有設定這個參數而報錯
        if getattr(cfg, 'use_global_disp_loss', False):
            # 1. 取得當前所有的控制點並連接成一個 Tensor
            curr_points = torch.cat(parameters["point"], dim=0)
            
            # 2. 計算每個點相對於初始位置的位移量 (距離)
            displacement = torch.norm(curr_points - init_points, dim=-1)
            
            # 3. 計算位移量的變異數 (Variance)。變異數越大代表變形越不均勻，要給予懲罰！
            loss_global_disp = torch.var(displacement)
            
            if cfg.use_wandb:
                wandb.log({"loss_global_disp": loss_global_disp}, step=step)
            
            # 4. 將懲罰加入總 Loss
            global_disp_w = getattr(cfg, 'global_disp_loss_w', 10.0)
            loss = loss + (loss_global_disp * global_disp_w)
        # -------------------------------------------------------
        # print(loss_laplacian_coordinates)

        #t_range.set_postfix({"loss": loss.item()})
        loss_msg = f"Step {step:04d} | 總和(Total): {loss.item():.2f}"
        if cfg.fclip_loss: 
            loss_msg += f" | 動力 CLIP: {loss_fclip.item():.2f}"
        if cfg.use_conformal_loss: 
            loss_msg += f" | 阻力角度(Conf): {loss_angles.item():.2f}"
        if cfg.use_Xing_loss: 
            loss_msg += f" | 防交叉(Xing): {loss_Xing.item():.2f}"
        if cfg.use_laplacian_loss: 
            loss_msg += f" | 平滑度(Lap): {loss_laplacian_coordinates.item():.2f}"
    ...
    # ── [改寫] 最終量化指標 ───────────────────────────────
    # 舊版 final_clip_loss = loss_fclip.item() 語意不明（含 -cos×scheduler 權重、且為 aug 後瞬間值）。
    # 改為：訓練後用「乾淨、無 augmentation」的最終字形，純評估算方向對齊度。
    #   final_dir_cos : cos(Δimage, Δtext)，越高=越貼合語意變化方向（主指標，與 direction loss 一致）
    #   final_abs_cos : cos(image, text)，越高=越接近目標 prompt（輔助指標）
    #   final_clip_loss: 保留舊欄位（最後一步 loss_fclip 瞬間值）以利與舊資料對照，但不建議作為論文主指標
    final_dir_cos = None
    final_abs_cos = None
    if cfg.fclip_loss:
        try:
            with torch.no_grad():
                # 用最後一次乾淨渲染圖 x（第784行生成，非 aug 版），取 batch 第一張即可
                x_clean = x[:1] if 'x' in dir() else None
                if x_clean is not None:
                    _metrics = fclip_loss.eval_metrics(x_clean)
                    final_dir_cos = _metrics.get("dir_cos", None)
                    final_abs_cos = _metrics.get("abs_cos", None)
        except Exception as _e:
            print(f"[warn] eval_metrics 失敗：{_e}")

    final_clip_loss = loss_fclip.item() if cfg.fclip_loss else None
    print(f"[final] dir_cos={final_dir_cos}  abs_cos={final_abs_cos}  legacy_loss={final_clip_loss}")
    return {
        "final_dir_cos": final_dir_cos,
        "final_abs_cos": final_abs_cos,
        "final_clip_loss_legacy": final_clip_loss,
    }
